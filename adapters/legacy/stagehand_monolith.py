"""Legacy Stagehand compatibility surface.

New Jobops execution must use :mod:`adapters.generic_ai`, whose production
pipeline is split into observer, fingerprinter, resolver, executor, verifier,
and cache modules.  This module remains temporarily for the original MR.Jobs
API and its downstream callers; it must not be selected by the new registry.
