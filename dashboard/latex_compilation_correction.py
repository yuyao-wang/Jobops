"""Authenticated UI adapter for typed LaTeX Compilation correction."""
...
LatexCompilationCorrectionCallable = Callable[
    ...,
    LatexCompilationCorrectionResult
    | Awaitable[LatexCompilationCorrectionResult],
]


class LatexCompilationCorrectionUIController:
    def __init__(
        self,
        *,
        correction_callable: LatexCompilationCorrectionCallable,
        ...
        *,
        ...
        value = self._correction_callable(
            LatexCompilationCorrectionCommand(
                ...
                action=LatexCompilationCorrectionAction(action),
                now=now,
            )
        )
        result = await value if inspect.isawaitable(value) else value
        if not isinstance(result, LatexCompilationCorrectionResult):
            raise ValueError("correction service returned an invalid result")
        return result


__all__ = [
    "LatexCompilationCorrectionCallable",
