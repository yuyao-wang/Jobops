"""Cover-letter evidence snapshots derived only from a faithful resume source.

This contract is deliberately independent of the resume-tailoring evidence in
:mod:`core.candidate_evidence`. That module's ``RESUME_TAILORING`` scope is
left untouched: authorising a fact for a cover letter is an explicit, separate
decision, and the scope participates in every evidence identity here so a
cover-letter item can never be confused with a tailoring one.
"""
...
COVER_LETTER_EVIDENCE_CONTRACT_VERSION = "cover-letter-evidence-v1"

_HASH_PATTERN = re.compile(r"^[a-f0-9]{64}$")
_EVIDENCE_ID_PATTERN = re.compile(
    r"^cover-letter-evidence-[a-f0-9]{64}$"
)
_SNAPSHOT_ID_PATTERN = re.compile(
    r"^cover-letter-evidence-snapshot-[a-f0-9]{64}$"
)
...
MAX_COVER_LETTER_EVIDENCE_CHARS = 20_000


class CoverLetterEvidenceSourceType(str, Enum):
    SOURCE_RESUME_PROJECTION = "SOURCE_RESUME_PROJECTION"


class CoverLetterEvidenceSensitivity(str, Enum):
    PERSONAL = "PERSONAL"


class CoverLetterEvidenceScope(str, Enum):
    """Its own scope. Cover-letter use is never inherited from tailoring."""

    COVER_LETTER = "COVER_LETTER"


class CoverLetterEvidenceVerificationStatus(str, Enum):
    USER_PROVIDED_DOCUMENT_STATEMENT = "USER_PROVIDED_DOCUMENT_STATEMENT"


class CoverLetterEvidenceSnapshotWriteStatus(str, Enum):
    CREATED = "CREATED"
    UNCHANGED = "UNCHANGED"
    FAILED = "FAILED"


class CoverLetterEvidenceSnapshotReadStatus(str, Enum):
    FOUND = "FOUND"
    NOT_FOUND = "NOT_FOUND"
    INTEGRITY_FAILURE = "INTEGRITY_FAILURE"


class CoverLetterEvidenceSnapshotStatus(str, Enum):
    CREATED = "CREATED"
    UNCHANGED = "UNCHANGED"
...
class CoverLetterEvidenceFailureReason(str, Enum):
...
def _evidence_text(value: Any) -> str:
    if (
        ...
        or not value.strip()
        or len(value) > MAX_COVER_LETTER_EVIDENCE_CHARS
    ):
...
def _evidence_identity_payload(
    *,
    ...
    allowed_scope: CoverLetterEvidenceScope,
...
def cover_letter_evidence_id(
    *,
    ...
    contract_version: str = COVER_LETTER_EVIDENCE_CONTRACT_VERSION,
) -> str:
    """Identity binds the scope, so cover-letter IDs never collide with others."""
    ...
    return (
        ...
        + _canonical_hash(
            _evidence_identity_payload(
                ...
                source_projection_hash=source_projection_hash,
                source_anchor_id=source_anchor_id,
                allowed_scope=CoverLetterEvidenceScope(allowed_scope),
                contract_version=contract_version,
            )
        )
    )
...
class CoverLetterEvidenceItem:
    evidence_id: str
    subject_id: str
    order: int
    evidence_text: str
    source_type: CoverLetterEvidenceSourceType
    source_projection_id: str
    source_projection_hash: str
    ...
    sensitivity: CoverLetterEvidenceSensitivity
    allowed_scopes: tuple[CoverLetterEvidenceScope, ...]
    verification_status: CoverLetterEvidenceVerificationStatus
    ...
    def __post_init__(self) -> None:
        ...
        source_type = CoverLetterEvidenceSourceType(self.source_type)
        ...
        sensitivity = CoverLetterEvidenceSensitivity(self.sensitivity)
        if (
            not isinstance(self.allowed_scopes, tuple)
            or tuple(
                CoverLetterEvidenceScope(item)
                for item in self.allowed_scopes
            )
            != (CoverLetterEvidenceScope.COVER_LETTER,)
        ):
            raise ValueError("cover-letter evidence scope is invalid")
        verification = CoverLetterEvidenceVerificationStatus(
            self.verification_status
        )
        if (
            verification
            is not CoverLetterEvidenceVerificationStatus
            .USER_PROVIDED_DOCUMENT_STATEMENT
        ):
            raise ValueError("cover-letter verification status is invalid")
        ...
        expected_id = cover_letter_evidence_id(
            subject_id=subject_id,
...
def _create_evidence_item(
    *,
    ...
    evidence_id = cover_letter_evidence_id(
        subject_id=subject_id,
    ...
    content = {
        ...
        "source_type": (
            CoverLetterEvidenceSourceType.SOURCE_RESUME_PROJECTION.value
        ),
        "source_projection_id": projection.projection_id,
        "source_projection_hash": projection.projection_content_hash,
        ...
        "sensitivity": CoverLetterEvidenceSensitivity.PERSONAL.value,
        "allowed_scopes": [CoverLetterEvidenceScope.COVER_LETTER.value],
        "verification_status": (
            CoverLetterEvidenceVerificationStatus
            .USER_PROVIDED_DOCUMENT_STATEMENT.value
        ),
        "verified_at": None,
        "expires_at": None,
    }
    return CoverLetterEvidenceItem(
        evidence_id=evidence_id,
        subject_id=subject_id,
        order=order,
        evidence_text=block.text,
        source_type=(
            CoverLetterEvidenceSourceType.SOURCE_RESUME_PROJECTION
        ),
        source_projection_id=projection.projection_id,
        source_projection_hash=projection.projection_content_hash,
        source_section_id=section_id,
        ...
        sensitivity=CoverLetterEvidenceSensitivity.PERSONAL,
        allowed_scopes=(CoverLetterEvidenceScope.COVER_LETTER,),
        verification_status=(
            CoverLetterEvidenceVerificationStatus
            .USER_PROVIDED_DOCUMENT_STATEMENT
        ),
...
def cover_letter_evidence_snapshot_id(**values: Any) -> str:
    return "cover-letter-evidence-snapshot-" + _canonical_hash(
        _snapshot_identity_payload(**values)
    )


@dataclass(frozen=True, slots=True)
class CoverLetterEvidenceSnapshot:
    snapshot_id: str
    contract_version: str
    subject_id: str
    application_plan_id: str
    ...
    source_projection_id: str
    source_projection_hash: str
    evidence_items: tuple[CoverLetterEvidenceItem, ...]
    ...
    def __post_init__(self) -> None:
        ...
        if contract != COVER_LETTER_EVIDENCE_CONTRACT_VERSION:
            raise ValueError("cover-letter evidence contract is unsupported")
        ...
        if (
            ...
            or any(
                not isinstance(item, CoverLetterEvidenceItem)
        ...
        expected_id = cover_letter_evidence_snapshot_id(
        ...
        object.__setattr__(self, "source_artifact_sha256", artifact_hash)
        object.__setattr__(self, "source_projection_id", projection_id)
        object.__setattr__(self, "source_projection_hash", projection_hash)
...
@dataclass(frozen=True, slots=True)
class CoverLetterEvidenceSnapshotWriteResult:
    status: CoverLetterEvidenceSnapshotWriteStatus
    snapshot: CoverLetterEvidenceSnapshot | None
    reason_code: CoverLetterEvidenceFailureReason | None
    retryable: bool

    def __post_init__(self) -> None:
        status = CoverLetterEvidenceSnapshotWriteStatus(self.status)
        object.__setattr__(self, "status", status)
        if self.reason_code is not None:
            object.__setattr__(
                self,
                "reason_code",
                CoverLetterEvidenceFailureReason(self.reason_code),
            )
        if type(self.retryable) is not bool:
            raise TypeError("retryable must be a boolean")
        if status in {
            CoverLetterEvidenceSnapshotWriteStatus.CREATED,
            CoverLetterEvidenceSnapshotWriteStatus.UNCHANGED,
        }:
            if (
                not isinstance(self.snapshot, CoverLetterEvidenceSnapshot)
...
@dataclass(frozen=True, slots=True)
class CoverLetterEvidenceSnapshotReadResult:
    status: CoverLetterEvidenceSnapshotReadStatus
    snapshot: CoverLetterEvidenceSnapshot | None
    reason_code: CoverLetterEvidenceFailureReason | None = None

    def __post_init__(self) -> None:
        status = CoverLetterEvidenceSnapshotReadStatus(self.status)
        object.__setattr__(self, "status", status)
        if self.reason_code is not None:
            object.__setattr__(
                self,
        ...
        if status is CoverLetterEvidenceSnapshotReadStatus.FOUND:
            if (
                not isinstance(self.snapshot, CoverLetterEvidenceSnapshot)
                or self.reason_code is not None
            ):
                raise ValueError("found snapshot read result is invalid")
        elif status is CoverLetterEvidenceSnapshotReadStatus.NOT_FOUND:
            ...
            or self.reason_code
            is not CoverLetterEvidenceFailureReason.SNAPSHOT_INTEGRITY_FAILURE
        ):
            raise ValueError("integrity-failure snapshot read is invalid")


@runtime_checkable
class CoverLetterEvidenceSnapshotRepository(Protocol):
    def save(
        self, snapshot: CoverLetterEvidenceSnapshot
    ) -> CoverLetterEvidenceSnapshotWriteResult:
        """Persist one immutable cover-letter evidence snapshot."""

    def get(
        self, *, subject_id: str, snapshot_id: str
    ) -> CoverLetterEvidenceSnapshotReadResult:
        """Read one subject-owned cover-letter evidence snapshot."""
...
def _item_from_dict(
    ...
    if (
        ...
        raise ValueError("persisted CoverLetterEvidenceItem is invalid")
    ...
    return CoverLetterEvidenceItem(
        ...
        source_type=CoverLetterEvidenceSourceType(value["source_type"]),
        ...
        sensitivity=CoverLetterEvidenceSensitivity(value["sensitivity"]),
        allowed_scopes=tuple(
            CoverLetterEvidenceScope(scope)
...
def _snapshot_from_dict(value: Any) -> CoverLetterEvidenceSnapshot:
    expected = {
        "snapshot_id",
        "contract_version",
        "subject_id",
    ...
    if (
        ...
        raise ValueError("persisted CoverLetterEvidenceSnapshot is invalid")
    return CoverLetterEvidenceSnapshot(
...
class PrivateHomeCoverLetterEvidenceSnapshotRepository:
    """Immutable, subject-scoped cover-letter evidence in Private Home."""
    ...
    def _path(self, subject_id: str, snapshot_id: str) -> Path:
        ...
        return (
            self._home.paths.cover_letter_evidence_snapshots
        ...
        with self._lock:
            if not path.exists():
                return CoverLetterEvidenceSnapshotReadResult(
                    status=(
                        CoverLetterEvidenceSnapshotReadStatus.NOT_FOUND
                    ),
                    snapshot=None,
                )
    ...
    def save(
        ...
        if not isinstance(snapshot, CoverLetterEvidenceSnapshot):
            raise TypeError(
                "snapshot must be a CoverLetterEvidenceSnapshot"
            )
        ...
        with self._lock:
            try:
                ...
                return CoverLetterEvidenceSnapshotWriteResult(
                    status=CoverLetterEvidenceSnapshotWriteStatus.FAILED,
                    snapshot=None,
                    reason_code=(
                        CoverLetterEvidenceFailureReason
            ...
            if created:
                return CoverLetterEvidenceSnapshotWriteResult(
                    status=CoverLetterEvidenceSnapshotWriteStatus.CREATED,
                    snapshot=snapshot,
                    reason_code=None,
                    retryable=False,
                )
            ...
            if (
                existing.status
                is CoverLetterEvidenceSnapshotReadStatus.FOUND
...
@dataclass(frozen=True, slots=True)
class CreateCoverLetterEvidenceSnapshotCommand:
    subject_id: str
    application_plan_id: str
    now: datetime
...
@dataclass(frozen=True, slots=True)
class CreateCoverLetterEvidenceSnapshotResult:
    status: CoverLetterEvidenceSnapshotStatus
    subject_id: str
    application_plan_id: str
    snapshot: CoverLetterEvidenceSnapshot | None
    write_result: CoverLetterEvidenceSnapshotWriteResult | None
    reason_code: CoverLetterEvidenceFailureReason | None
    retryable: bool
    message: str

    def __post_init__(self) -> None:
        status = CoverLetterEvidenceSnapshotStatus(self.status)
        object.__setattr__(self, "status", status)
        if self.reason_code is not None:
            object.__setattr__(
                self,
        ...
        if status in {
            ...
            expected = CoverLetterEvidenceSnapshotWriteStatus(status.value)
            if (
                not isinstance(self.snapshot, CoverLetterEvidenceSnapshot)
                or not isinstance(
                    self.write_result,
        ...
        elif status is CoverLetterEvidenceSnapshotStatus.DEFERRED_NO_EVIDENCE:
            if (
                self.snapshot is not None
...
def _failure(
    command: CreateCoverLetterEvidenceSnapshotCommand,
    reason: CoverLetterEvidenceFailureReason,
    *,
    retryable: bool = False,
    write_result: CoverLetterEvidenceSnapshotWriteResult | None = None,
) -> CreateCoverLetterEvidenceSnapshotResult:
    return CreateCoverLetterEvidenceSnapshotResult(
        status=CoverLetterEvidenceSnapshotStatus.FAILED,
        subject_id=(
            command.subject_id
        ...
        reason_code=reason,
        retryable=retryable,
        message=f"Cover-letter evidence snapshot failed: {reason.value}.",
    )
...
def create_cover_letter_evidence_snapshot(
    command: CreateCoverLetterEvidenceSnapshotCommand,
    *,
    ...
    snapshot_repository: CoverLetterEvidenceSnapshotRepository,
) -> CreateCoverLetterEvidenceSnapshotResult:
    """Authorize existing resume-source facts for cover-letter use only."""
    ...
    try:
        ...
        return _failure(
            command, CoverLetterEvidenceFailureReason.INVALID_REQUEST
        )
    try:
    ...
    if plan_result.status is ApplicationPlanReadStatus.NOT_FOUND:
        return _failure(
            command,
            CoverLetterEvidenceFailureReason.APPLICATION_PLAN_NOT_FOUND,
        )
    if (
    ...
    if selection_result.status is ResumeSelectionDecisionReadStatus.NOT_FOUND:
        return _failure(
            command,
            CoverLetterEvidenceFailureReason.RESUME_SELECTION_NOT_FOUND,
        )
    if (
    ...
    if candidate_result.status is ResumeCandidateReadStatus.NOT_FOUND:
        return _failure(
            command,
            CoverLetterEvidenceFailureReason.RESUME_CANDIDATE_NOT_FOUND,
        )
    if (
    ...
    if projection_result.status is SourceResumeProjectionReadStatus.NOT_FOUND:
        return _failure(
            command,
            CoverLetterEvidenceFailureReason.SOURCE_PROJECTION_NOT_FOUND,
        )
    if (
    ...
    try:
        ...
        return _failure(
            command, CoverLetterEvidenceFailureReason.EVIDENCE_INVALID
        )
    if not items:
        return CreateCoverLetterEvidenceSnapshotResult(
            status=(
                CoverLetterEvidenceSnapshotStatus.DEFERRED_NO_EVIDENCE
            ),
            subject_id=subject_id,
            application_plan_id=plan_id,
            snapshot=None,
            ...
            retryable=False,
            message=(
                "The source resume projection contains no usable "
                "cover-letter evidence."
            ),
        )
    identity = {
        "contract_version": COVER_LETTER_EVIDENCE_CONTRACT_VERSION,
        "subject_id": subject_id,
    ...
    snapshot_id = cover_letter_evidence_snapshot_id(**identity)
    try:
        ...
        return _failure(
            command,
            CoverLetterEvidenceFailureReason.SNAPSHOT_INTEGRITY_FAILURE,
        )
    if (
        existing.status
        is CoverLetterEvidenceSnapshotReadStatus.INTEGRITY_FAILURE
    ):
        return _failure(
            command,
            CoverLetterEvidenceFailureReason.SNAPSHOT_INTEGRITY_FAILURE,
    ...
    if (
        existing.status is CoverLetterEvidenceSnapshotReadStatus.FOUND
        and existing.snapshot is not None
    ):
        write_result = CoverLetterEvidenceSnapshotWriteResult(
            status=CoverLetterEvidenceSnapshotWriteStatus.UNCHANGED,
            snapshot=existing.snapshot,
            reason_code=None,
            retryable=False,
        )
        return CreateCoverLetterEvidenceSnapshotResult(
            status=CoverLetterEvidenceSnapshotStatus.UNCHANGED,
            subject_id=subject_id,
            application_plan_id=plan_id,
            snapshot=existing.snapshot,
            write_result=write_result,
            ...
            retryable=False,
            message=(
                "The existing cover-letter evidence snapshot is unchanged."
            ),
        )
    snapshot_content = {
        "snapshot_id": snapshot_id,
    ...
    try:
        ...
        return _failure(
            command,
            CoverLetterEvidenceFailureReason.SNAPSHOT_PERSISTENCE_FAILED,
            retryable=True,
        )
    if write_result.status is CoverLetterEvidenceSnapshotWriteStatus.FAILED:
        return _failure(
            command,
            write_result.reason_code
            or CoverLetterEvidenceFailureReason.SNAPSHOT_PERSISTENCE_FAILED,
            retryable=write_result.retryable,
            write_result=write_result,
        )
    result_status = CoverLetterEvidenceSnapshotStatus(
    ...
    return CreateCoverLetterEvidenceSnapshotResult(
        ...
        retryable=False,
        message=(
            "The cover-letter evidence snapshot was created."
            if result_status is CoverLetterEvidenceSnapshotStatus.CREATED
            else "The existing cover-letter evidence snapshot is unchanged."
        ),
    )
...
__all__ = [
    ...
    "CoverLetterEvidenceSensitivity",
    ...
    "CoverLetterEvidenceSnapshotWriteResult",
    "CoverLetterEvidenceSnapshotWriteStatus",
    "CoverLetterEvidenceSourceType",
    "CoverLetterEvidenceVerificationStatus",
    "CreateCoverLetterEvidenceSnapshotCommand",
    "CreateCoverLetterEvidenceSnapshotResult",
    "MAX_COVER_LETTER_EVIDENCE_CHARS",
