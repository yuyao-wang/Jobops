...
from core.candidate_evidence import CandidateEvidenceScope
...
def _create(parts, *, subject_id: str = "subject-a", now: datetime = NOW):
    return create_cover_letter_evidence_snapshot(
        CreateCoverLetterEvidenceSnapshotCommand(
            subject_id=subject_id,
...
def test_snapshot_builds_ordered_exact_typed_evidence(
    tmp_path: Path,
) -> None:
    ...
    assert result.status is CoverLetterEvidenceSnapshotStatus.CREATED
    snapshot = result.snapshot
    assert snapshot is not None
    source = _projection_blocks(parts["projection"])
    assert tuple(item.order for item in snapshot.evidence_items) == (
        0,
        1,
        2,
    )
    assert tuple(
        item.evidence_text for item in snapshot.evidence_items
    ) == tuple(block.text for _, block in source)
...
def test_scope_is_cover_letter_and_never_reuses_resume_tailoring(
    tmp_path: Path,
) -> None:
    ...
    for item in result.snapshot.evidence_items:
        ...
        assert all(
            scope is not CandidateEvidenceScope.RESUME_TAILORING
            for scope in item.allowed_scopes
        )
        assert (
            item.source_type
            is CoverLetterEvidenceSourceType.SOURCE_RESUME_PROJECTION
        )
        assert item.sensitivity is CoverLetterEvidenceSensitivity.PERSONAL
        assert (
            item.verification_status
            is CoverLetterEvidenceVerificationStatus
...
def test_evidence_ids_are_distinct_from_resume_tailoring_ids(
    tmp_path: Path,
) -> None:
    ...
    parts = _setup(tmp_path)
    cover_letter = _create(parts)
    tailoring = create_candidate_evidence_snapshot(
        CreateCandidateEvidenceSnapshotCommand(
        ...
        candidate_repository=parts["candidate_repository"],
        projection_repository=parts["projection_repository"],
        snapshot_repository=PrivateHomeCandidateEvidenceSnapshotRepository(
            parts["home"]
        ),
    )

    assert cover_letter.snapshot is not None
    assert tailoring.snapshot is not None
    cover_letter_ids = {
        item.evidence_id for item in cover_letter.snapshot.evidence_items
    }
    tailoring_ids = {
        item.evidence_id for item in tailoring.snapshot.evidence_items
    }
    assert cover_letter_ids.isdisjoint(tailoring_ids)
    assert cover_letter.snapshot.snapshot_id != tailoring.snapshot.snapshot_id
...
def test_replay_and_restart_preserve_ids_hashes_and_created_time(
    ...
    assert replay.status is CoverLetterEvidenceSnapshotStatus.UNCHANGED
...
@pytest.mark.parametrize(
    ...
    [
        (
            "selection",
            "job_content_hash",
            CoverLetterEvidenceFailureReason.RESUME_SELECTION_BINDING_MISMATCH,
        ),
        (
            "candidate",
            "artifact_sha256",
            CoverLetterEvidenceFailureReason.RESUME_CANDIDATE_BINDING_MISMATCH,
        ),
        (
            "projection",
            "artifact_sha256",
            CoverLetterEvidenceFailureReason.SOURCE_PROJECTION_BINDING_MISMATCH,
        ),
    ],
)
def test_binding_mismatch_fails_without_snapshot(
    ...
    assert result.status is CoverLetterEvidenceSnapshotStatus.FAILED
    assert result.reason_code is reason
    assert not tuple(
        parts["home"].paths.cover_letter_evidence_snapshots.rglob("*.json")
    )
...
def test_missing_or_changed_locator_and_projection_hash_fail_closed(
    ...
    assert (
        result.reason_code
        is CoverLetterEvidenceFailureReason.SOURCE_PROJECTION_BINDING_MISMATCH
    )
...
def test_empty_projection_defers_without_snapshot(tmp_path: Path) -> None:
    ...
    assert (
        result.status
        is CoverLetterEvidenceSnapshotStatus.DEFERRED_NO_EVIDENCE
    )
...
def test_new_plan_selection_or_projection_creates_new_snapshot(
    ...
    assert selection_changed.status is (
...
def test_contract_version_changes_identity_without_using_time() -> None:
    ...
    first = cover_letter_evidence_snapshot_id(
        contract_version="cover-letter-evidence-v1", **common
    )
    changed = cover_letter_evidence_snapshot_id(
        contract_version="cover-letter-evidence-v2", **common
    )
...
def test_subject_isolation_and_restart_read(tmp_path: Path) -> None:
    ...
    repository = PrivateHomeCoverLetterEvidenceSnapshotRepository(
    ...
    assert cross.status is CoverLetterEvidenceSnapshotReadStatus.NOT_FOUND
    assert read.status is CoverLetterEvidenceSnapshotReadStatus.FOUND
...
def test_repository_conflict_corruption_and_duplicate_evidence_fail_closed(
    ...
    record = next(
        parts["home"].paths.cover_letter_evidence_snapshots.rglob(
    ...
    assert conflict.status is CoverLetterEvidenceSnapshotWriteStatus.FAILED
    assert (
        conflict.reason_code
        is CoverLetterEvidenceFailureReason.SNAPSHOT_INTEGRITY_FAILURE
    )
...
def test_invalid_command_and_missing_upstream_fail_without_snapshot(
    ...
    parts = _setup(tmp_path)
    naive = _create(parts, now=datetime(2026, 7, 31, 10, 0))
    missing = create_cover_letter_evidence_snapshot(
        CreateCoverLetterEvidenceSnapshotCommand(
            subject_id="subject-a",
    ...
    assert (
        naive.reason_code
        is CoverLetterEvidenceFailureReason.INVALID_REQUEST
    )
    assert (
        missing.reason_code
        is CoverLetterEvidenceFailureReason.APPLICATION_PLAN_NOT_FOUND
    )


def test_module_has_no_jd_agent_tailoring_or_execution_dependency() -> None:
...
def test_module_does_not_import_resume_tailoring_scope() -> None:
    module_path = Path(evidence_module.__file__)
    tree = ast.parse(module_path.read_text(encoding="utf-8"))
    imported_names = {
        alias.name
        for node in ast.walk(tree)
        if isinstance(node, ast.ImportFrom)
        and (node.module or "").endswith("candidate_evidence")
        for alias in node.names
    }

    assert imported_names == set()
    assert not hasattr(evidence_module, "CandidateEvidenceScope")
